package P1301_1400.P1334_Find_the_City_With_the_Smallest_Number_of_Neighbors_at_a_Threshold_Distance;
...
public class Main {
    ...
    public int findTheCity(int n, int[][] edges, int distanceThreshold) {
        ...
        for (int i = 0; i < n; ++i) {
            Arrays.fill(dist[i], 10001); // Use a value larger than the max possible distanceThreshold
            dist[i][i] = 0;
        }

        // Populate initial distances with given edges
        for (int[] edge : edges) {
            int u = edge[0];
            int v = edge[1];
            int weight = edge[2];
...
//This solution efficiently finds the city with the smallest number of reachable cities within the given distance
// threshold using the Floyd-Warshall algorithm to compute shortest paths.
