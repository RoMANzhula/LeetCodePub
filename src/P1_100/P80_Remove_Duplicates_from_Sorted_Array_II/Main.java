package P1_100.P80_Remove_Duplicates_from_Sorted_Array_II;
...
public class Main {
    ...
    public int removeDuplicates(int[] nums) {
        int n = nums.length;
        if (n <= 2) {
            return n; //no need to remove duplicates if there are 0 or 1 elements, or if the array is already
            // correctly formatted
        }
        ...
        for (int i = 1; i < n; i++) {
            ...
            if (count <= 2) {
                nums[index] = nums[i]; //place the unique element in the next position
                index++;
            }
        }
