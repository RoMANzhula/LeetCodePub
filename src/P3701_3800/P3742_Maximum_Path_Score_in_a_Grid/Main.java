package P3701_3800.P3742_Maximum_Path_Score_in_a_Grid;

import java.util.Arrays;

public class Main {

    public static void main(String[] args) {
        Main solution = new Main();

        int[][] grid1 = {{0, 1}, {2, 0}};
        int k1 = 1;
        System.out.println(solution.maxPathScore(grid1, k1)); // 2

        int[][] grid2 = {{0, 1}, {1, 2}};
        int k2 = 1;
        System.out.println(solution.maxPathScore(grid2, k2)); // -1
    }

    public int maxPathScore(int[][] grid, int k) {
        int m = grid.length;
        int n = grid[0].length;

        // dp[i][j][c] = max score at (i,j) with cost c
        int[][][] dp = new int[m][n][k + 1];

        // initialize with -1 (unreachable)
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {
                Arrays.fill(dp[i][j], -1);
            }
        }

        // start position
        dp[0][0][0] = 0;

        for (int i = 0; i < m; i++) {
            for (int j = 0; j < n; j++) {

                for (int c = 0; c <= k; c++) {
                    if (dp[i][j][c] == -1) continue;

                    // move right
                    if (j + 1 < n) {
                        process(grid, dp, i, j, i, j + 1, c, k);
                    }

                    // move down
                    if (i + 1 < m) {
                        process(grid, dp, i, j, i + 1, j, c, k);
                    }
                }
            }
        }

        int res = -1;
        for (int c = 0; c <= k; c++) {
            res = Math.max(res, dp[m - 1][n - 1][c]);
        }

        return res;
    }

    private void process(
            int[][] grid,
            int[][][] dp,
            int i,
            int j,
            int ni,
            int nj,
            int costSoFar,
            int k
    ) {

        int val = grid[ni][nj];

        int addScore = val;
        int addCost = (val == 0) ? 0 : 1;

        int newCost = costSoFar + addCost;
        if (newCost > k) return;

        dp[ni][nj][newCost] = Math.max(
                dp[ni][nj][newCost],
                dp[i][j][costSoFar] + addScore
        );
    }

}

//Complexity:
// time and space - O(m * n * k)


//You are given an m x n grid where each cell contains one of the values 0, 1, or 2. You are also given an integer k.
//You start from the top-left corner (0, 0) and want to reach the bottom-right corner (m - 1, n - 1) by moving only
// right or down.
//Each cell contributes a specific score and incurs an associated cost, according to their cell values:
//0: adds 0 to your score and costs 0.
//1: adds 1 to your score and costs 1.
//2: adds 2 to your score and costs 1.
//Return the maximum score achievable without exceeding a total cost of k, or -1 if no valid path exists.
//Note: If you reach the last cell but the total cost exceeds k, the path is invalid.

//Example 1:
//Input: grid = [[0, 1],[2, 0]], k = 1
//Output: 2
//Explanation:
//The optimal path is:
//Cell	   grid[i][j]	Score	Total Score	  Cost	Total Cost
//(0, 0)	   0	      0          0	       0	      0
//(1, 0)	   2	      2          2	       1	      1
//(1, 1)	   0	      0          2	       0	      1
//Thus, the maximum possible score is 2.

//Example 2:
//Input: grid = [[0, 1],[1, 2]], k = 1
//Output: -1
//Explanation:
//There is no path that reaches cell (1, 1) without exceeding cost k. Thus, the answer is -1.

//Constraints:
//1 <= m, n <= 200
//0 <= k <= 103
//grid[0][0] == 0
//0 <= grid[i][j] <= 2
