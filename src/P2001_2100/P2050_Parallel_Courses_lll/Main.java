...
public class Main {
    ...
    public static int minimumTime(int n, int[][] relations, int[] time) {
        ...
        int result = 0;
        for (int i = 1; i <= n; i++) {
            result = Math.max(result, completionTime[i] + time[i - 1]);
        }
...
// integer array relations where relations[j] = [prevCoursej, nextCoursej] denotes that course prevCoursej has to
