package P1901_2000.P1948_Delete_Duplicate_Folders_in_System;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<List<String>> remaining = solution.deleteDuplicateFolder(paths);

        for (List<String> path : remaining) {
    ...
    private String serialize(Node node) {
        ...
        for (Node child : node.children.values()) {
            ...
            serializedChildren.add(child.name + "(" + serial + ")");
        }

        Collections.sort(serializedChildren); // to avoid order sensitivity
    ...
    // collect valid paths (non-deleted)
...
//Complexity:
// time - O(N * L log L)    - here N = number of folders, L = average path length (log L due to sorting children)
// space - O(N * L)


//Due to a bug, there are many duplicate folders in a file system. You are given a 2D array paths, where
// paths[i] is an array representing an absolute path to the ith folder in the file system.
//For example, ["one", "two", "three"] represents the path "/one/two/three".
//Two folders (not necessarily on the same level) are identical if they contain the same non-empty set of
// identical subfolders and underlying subfolder structure. The folders do not need to be at the root level to
// be identical. If two or more folders are identical, then mark the folders as well as all their subfolders.
...
// file system only runs the deletion once, so any folders that become identical after the initial deletion are
// not deleted.
//
...
//Output: [["c"],["c","b"],["a"],["a","b"]]
//Explanation: The file structure is as shown.
//Folders "/a/b/x" and "/w" (and their subfolders) are marked for deletion because they both contain an
// empty folder named "y".
//Note that folders "/a" and "/c" are identical after the deletion, but they are not deleted because they were
// not marked beforehand.
