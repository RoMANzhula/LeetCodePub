package P1801_1900.P1823_Find_the_Winner_of_Circular_Game;
...
public class Main {
    public static void main(String[] args) {
        ...
        int n1 = 5, k1 = 2;
        System.out.println("The winner is: " + solution.findTheWinner(n1, k1));  // Output: 3

        int n2 = 6, k2 = 5;
        System.out.println("The winner is: " + solution.findTheWinner(n2, k2));  // Output: 1
    }

    public int findTheWinner(int n, int k) {
        int winner = 0;  // Base case: when there's only one person, they are the winner.

        // We simulate the elimination process starting with 2 up to n persons
        for (int i = 1; i <= n; i++) {
            winner = (winner + k) % i;
        }
...
// clockwise order. More formally, moving clockwise from the ith friend brings you to the (i+1)th friend
