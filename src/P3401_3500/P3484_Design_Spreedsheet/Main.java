package P3401_3500.P3484_Design_Spreedsheet;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.getValue("=5+7")); // 12
        solution.setCell("A1", 10);
        System.out.println(solution.getValue("=A1+6")); // 16
        solution.setCell("B2", 15);
        System.out.println(solution.getValue("=A1+B2")); // 25
        solution.resetCell("A1");
        System.out.println(solution.getValue("=A1+B2")); // 15
    }

    private int[][] grid;  // rows x 26

    public Main(int rows) {
        grid = new int[rows][26]; // initialized to 0
    }
    ...
    private int[] parseCell(String cell) {
        ...
        int row = Integer.parseInt(cell.substring(1)) - 1; // 1-based -> 0-based
...
//A spreadsheet is a grid with 26 columns (labeled from 'A' to 'Z') and a given number of rows. Each cell in
// the spreadsheet can hold an integer value between 0 and 105.
