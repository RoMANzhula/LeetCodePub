package P2901_3000.P2901_Longest_Unequal_Adjacent_Groups_Subsequence_II;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        List<String> res1 = solution.getWordsInLongestSubsequence(words1, groups1);
        System.out.println(res1); // Output: ["bab","cab"]

        String[] words2 = {"a", "b", "c", "d"};
        int[] groups2 = {1, 2, 3, 4};

        List<String> res2 = solution.getWordsInLongestSubsequence(words2, groups2);
        System.out.println(res2); // Output: ["a","b","c","d"]
    }

    public List<String> getWordsInLongestSubsequence(String[] words, int[] groups) {
        int len = words.length;
        int[] dp = new int[len];       // dp[i] = length of longest valid subsequence ending at i
        int[] prev = new int[len];     // prev[i] = previous index in the path
        Arrays.fill(dp, 1);
        Arrays.fill(prev, -1);
        ...
        // build the result from the back
...
//Complexity:
// time - O(n^2 * L) - n - number of words, L - max length of word
// space - O(n)
...
//Return a string array containing the words corresponding to the indices (in order) in the selected subsequence. If
